...
Created two Python files to clean and prepare the product and image data.
...
It's worth noting that the model overfitting was not overlooked at this stage. Knowing that the image model would be merged with the text model later, I chose to try and make the model fit as best as possible at this stage, but ultimately tackle any existing overfitting problems later instead.
...
- I decided to tokenize the text within the `__getitem__` method of my TextDatabase class. I did this because the tokens can be cast to tensors, and tensors can be loaded into memory by the GPU using the `.to(device)` command where device=cuda. This increases computation speed dramatically.
- For a similar reason, I kept the second stage of creating embeddings, using the BERT model, out of my `__getitem__` method and added it to my TextClassifier model class as an attribute instead. This meant that I could load the whole model, including the BERT model, with my GPU too, which was an even more significant speed improvement.
...
Below is a comparison of training a model using the dataset with only product descriptions for word embeddings (grey), and a model trained on word embeddings created from the combined columns (orange).
...
- The first four units consist of a similar structure: a Convultional layer, Activation layer (ReLU), Max Pooling, and a Dropout layer.
